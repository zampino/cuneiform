...
public class ForeignLambdaExpr extends LambdaExpr {
	...
	private static final String[] LABEL_LANG = {
		...
		LANGID_PYTHON, LANGID_PERL, LANGID_JAVA, LANGID_SCALA, LANGID_PEGASUS
	};
	...
	public ForeignLambdaExpr( Prototype prototype, int lang, String body ) {
		...
		if( lang < 0 || lang >= LABEL_LANG.length )
			throw new RuntimeException( "Language id "+lang+" not recognized. Must be a number in [0,"+( LABEL_LANG.length-1 )+"]." );
